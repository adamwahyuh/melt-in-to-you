            ...
            <div class="p-6 sm:p-8">
                <form action="{{ route('post.address.store', Auth::id()) }}" method="POST" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <x-form.input type="number" name="rt" label="RT" class="w-full" />
                        <x-form.input type="number" name="rw" label="RW" class="w-full" />
                    </div>

                    <!-- Grid untuk wilayah administratif -->
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <x-form.input name="kelurahan" label="Kelurahan / Desa" class="w-full" />
                        <x-form.input name="kecamatan" label="Kecamatan" class="w-full" />
                    </div>

                    <!-- Grid untuk Kota dan Kode Pos -->
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <x-form.input name="kota" label="Kota / Kabupaten" class="w-full" />
                        <x-form.input name="kode_pos" label="Kode Pos" class="w-full" />
                    </div>

                    <!-- Full width untuk alamat lengkap -->
                    <div>
                        <x-form.input name="alamat" label="Alamat Lengkap (Nama Jalan, Gedung, No. Rumah)" class="w-full" />
                    </div>
